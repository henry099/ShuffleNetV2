...
def bn_relu(name,out_channels):
    return nn.Sequential(
        g_name(name + '/bn', nn.BatchNorm2d(out_channels)),
        g_name(name + '/relu', nn.ReLU(inplace=True))
    )

def bn_prelu(name,out_channels,init_alpha=0.25):
    return nn.Sequential(
        g_name(name + '/bn', nn.BatchNorm2d(out_channels)),
        g_name(name + '/prelu', nn.PReLU(out_channels,init_alpha))
    )
...
def conv_bn_relu(name, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1):
    return nn.Sequential(
        ...
        g_name(name + '/bn', nn.BatchNorm2d(out_channels)),
        # g_name(name + '/relu', nn.ReLU(inplace=True)),
        g_name(name + '/relu', nn.LeakyReLU(0.2)),
    )
...
def conv(name, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
    return g_name(name, nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias))
